...
Class tniCPT {
	...
	function __construct($singular, $plural, $args = array() ) {
		...
		add_action('init', array( $this, 'tni_cpt' ));
		add_filter('post_updated_messages', array( $this, 'tni_cpt_messages'));
	}
	...
	function tni_cpt() {
		...
		// default $args
		if( !$args['description'] ) $args['description'] = 'Holds our ' . ucfirst($this->singular) . ' posts';
		if( !$args['public'] ) $args['public'] = true;
		if( !$args['menu_position'] ) $args['menu_position'] = 20;
		if( !$args['supports'] ) $args['supports'] = array('title','editor','thumbnail','excerpt','comments','page-attributes');
		if( !$args['has_archive'] ) $args['has_archive'] = true;
		if( !$args['hierarchical'] ) $args['hierarchical'] = true;
	...
	function tni_cpt_messages($messages){
...
$tni_cpts = get_option( 'tni_cpt' );
$tni_cpts = explode("|", preg_replace('/\r\n|\r|\n/m','|',$tni_cpts["tni_cpt_args"]));
foreach ($tni_cpts as $cpt) {
	$cpt = explode(',', $cpt);
	if( count($cpt) == 2 ) new tniCPT( $cpt[0], $cpt[1] );
	if( count($cpt) == 3 ) new tniCPT( $cpt[0], $cpt[1], $cpt[2] );
}
